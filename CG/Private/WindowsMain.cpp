...
uint32 CompileShaderFile(const TCHAR* FileName, const GLenum ShaderType)
{
	...
	{
		LE_DebugLogError(TEXT("Arquivo \'%s\' não encontrado.\n"), FileName);
	}

	if (ReadFile(FileHandle, FileContent, NumberOfBytesToRead - 1, &NumberOfBytesRead, NULL) == FALSE)
	{
		LE_DebugLogError(TEXT("Erro ao ler o arquivo \'%s\'.\n"), FileName);
	}
...
void LoadShader(const TCHAR* VertexShaderFileName, const TCHAR* FragmentShaderFileName, uint32* OutVertexShader, uint32* OutFragmentShader, uint32* OutShader)
{
...
FORCEINLINE void MainLoop()
{
	...
	while (!bQuit)
	{
		...
		glClearColor(12 / 255.0f, 19 / 255.0f, 23 / 255.0f, 0.0f);
...
int32 WINAPI WinMain(HINSTANCE InstanceHandle, HINSTANCE PrevInstanceHandle, LPSTR CommandLine, int CommandShow)
{
	...
	// Criar janela
	{
		...
		GWindowHandle = CreateWindow(ClassName, ClassName, Style, 0, 0, 1280, 720, NULL, NULL, InstanceHandle, NULL);
	}
